...
| **카페24 OAuth 연동** | 카페24 OAuth 2.0 인가코드 플로우 구현 · 토큰 발급/갱신 · 셀러 DB 등록 (`SellerOAuthService`, `Cafe24ApiClient`) — 코드 완성, 실 계정 검증은 client-id 파트너 승인 후 Phase 3 |
| **Admin 이미지 검수 파이프라인** | n8n 합성 룩북을 AI가 1차 자동 검수(콜백 수신) → Admin 최종 승인·거절 → 승인 시 캐시 무효화 후 게시되는 2단계 흐름 (`AdminInspectionCommandService`) |
...
**현재 구현 상태:** OAuth 플로우 코드 완성. 실제 카페24 client-id/secret은 파트너 승인이 필요해 실 계정 검증은 Phase 3. 상품 수집 로직도 Phase 3.
